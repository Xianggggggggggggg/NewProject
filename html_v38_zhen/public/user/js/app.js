...
async function beginInterview() {
 ...
 try {
        ...
        // 👈 關鍵修改：不直接去 interview.html，而是彈出成功訊息，並導向訊息/歷史紀錄頁面
        alert('🎉 履歷已成功送出！\n請前往「訊息中心」靜候企業的書審結果與面試通知。').then(() => {
             window.location.href = 'chat.html'; // 導向求職者的訊息中心/歷史頁面
        });

    } catch (err) {
        console.error('投遞履歷失敗:', err.message);
        alert('投遞失敗，原因：' + err.message);
    }
  }
      ...
      sessions.forEach(s => {
        ...
        if (s.status === 'status-1') {
            ...
            btn.addEventListener('click', () => {
                // 🌟 新增判斷：這筆面試紀錄是否有被分配到「多人房間 (room_id)」
                if (s.room_id) {
                    // 👉 有房間：導向多人面試 (group_interview.html)
                    // 備註：把 room_id 也帶上網址，這樣 group_interview.js 才知道要連哪個房間
                    window.location.href = `group_interview.html?session_id=${s.session_id}&room_id=${s.room_id}&resume_id=${s.resume_id}&position=${encodeURIComponent(s.position)}&type=${encodeURIComponent(s.type)}`;
                } else {
                    // 👉 沒有房間：維持原本的單人面試 (interview.html)
            ...
            // 狀態 3, 4, 5 或 已結束：查看報告
            btn.textContent = '📄 查看報告';
            btn.disabled = false;
            btn.addEventListener('click', () => {
                window.location.href = `hr_report.html?session_id=${s.session_id}`;
            });
        }
