...
// ==========================================
// 👥 團體面試專屬：單場多人面試綜合對比報告 API (無資料庫應急版)
// ==========================================
...
router.post('/group-rooms/:roomId/report', async (req, res) => {
    try {
        ...
        // 🌟 2. 資料瘦身：只挑出這個房間裡「已完成 AI 評分」的人
        const candidateList = [];
        (sessions || []).forEach(s => {
            let report = s.evaluation_reports;
            if (Array.isArray(report)) report = report[0];
            if (!report) return; // 沒報告跳過
        ...
        if (candidateList.length === 0) {
            return res.status(400).json({ success: false, error: '此房間的應徵者皆尚未產出個人報告，無法進行團面比較。' });
        }

        // 🌟 3. 專屬團體面試的 AI Prompt (強調同場競技與團隊動態)
        const prompt = `
你是一位資深招募顧問。這是一場「多人團體面試」的評估。
以下是參加同「一場」團體面試的候選人 AI 評估摘要（JSON 陣列）。
請針對這幾位在同一場表現的人選進行橫向對比。請「只回傳 JSON」，不要有任何 Markdown 符號，格式如下：
{
  "room_overview": "針對這場團體面試的整體氣氛與候選人總體素質的簡短總評（80字內）",
  "ranking": [
    { "name": "姓名", "overall_score": 數字, "reason": "在此場團面中的相對優勢或表現" }
  ],
  "best_communicator": "本場團面中溝通表達或團隊潛力最佳的人選姓名與理由",
  "standout_performer": "本場團面中技術或專業最突出的人選姓名與理由"
}
...
`;
        ...
        // 🌟 4. 呼叫 OpenAI 產生報告，算完直接回傳 (不寫入資料庫！)
        const reportJson = await callOpenAIForJson(prompt);
