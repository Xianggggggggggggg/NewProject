...
async function startInterviewAI() {
    ...
    try {
        ...
        ws.onopen = () => {
            ...
            transcriptBox.innerHTML += '<div class="ai-msg" style="padding:10px; background:#f0f0f0; border-radius:8px;">系統：連線成功，正在讀取履歷資料與生成考題...</div>';
        ...
        ws.onmessage = async (event) => {
            ...
            // 🌟 這裡有修改：多傳入 data.ai_role 給 appendTranscript
            if (data.customType === 'ai_transcript_final') {
                appendTranscript('ai', data.text, data.ai_role);
            }
...
window.addEventListener('load', () => {
    ...
    document.getElementById('transcriptBox').innerHTML = '<div class="ai-msg" style="padding:10px; background:#f0f0f0; border-radius:8px;">系統：請點擊畫面任何一處以啟動麥克風與相機...</div>';
});