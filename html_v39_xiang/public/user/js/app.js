      ...
      sessions.forEach(s => {
        ...
        if (s.status === 'status-1') {
            ...
            btn.addEventListener('click', () => {
                // 🌟 商業邏輯整合：HR 如果有分配 room_id，就去多人房間，否則去單人房間
                const targetUrl = s.room_id ? 'group_interview.html' : 'interview.html';
                
                // 自動帶入全部正確參數，不用再手動改網址！
                window.location.href = `${targetUrl}?session_id=${s.session_id}&resume_id=${s.resume_id}&position=${encodeURIComponent(s.position || '')}&type=${encodeURIComponent(s.type || '')}`;
            });
        } else {
            // 狀態 3, 4, 5 或 已結束：查看報告
