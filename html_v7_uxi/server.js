...
wss.on('connection', (clientWs) => {
    ...
    const startGeminiConnection = (dynamicSystemPrompt) => {
        ...
        // 🌟 1. 定義可用的面試官聲音清單 (包含各種男女聲)
        const availableVoices = ["Aoede", "Puck", "Kore", "Charon", "Fenrir"];

        // 🌟 2. 隨機抽出一個聲音，做為這場面試的專屬面試官
        const randomVoice = availableVoices[Math.floor(Math.random() * availableVoices.length)];
        console.log(`🎤 [系統] 本場面試官聲音已設定為: ${randomVoice}`);
                ...
                setup: {
                    ...
                    generationConfig: {
                        ...
                        speechConfig: {
                            voiceConfig: {
                                prebuiltVoiceConfig: {
                                    voiceName: randomVoice // 🌟 3. 套用剛剛隨機抽出來的聲音
                                }
                            }
                        }
                    },
                    realtime_input_config: { automatic_activity_detection: { silence_duration_ms: 2000 } }
                }
            }));
        });
        ...
        geminiWs.on('close', (code, reason) => {
            console.log(`\n❌ [Gemini 異常] Google 伺服器關閉了連線！`);
            console.log(`代碼: ${code}`);
            console.log(`原因: ${reason.toString() || '未提供具體原因'}`);

            try {
                clientWs.send(JSON.stringify({
                    customType: 'ai_transcript_final',
                    text: "（系統異常：AI 語音連線中斷，請重整頁面或稍後再試。）"
                }));
            } catch (e) { }
        });

        geminiWs.on('error', (err) => {
            console.error(`\n❌ [Gemini 錯誤] 發生嚴重連線錯誤:`, err.message);
        });
    ...
    clientWs.on('message', async (msg) => {
        try {
            ...
            if (parsedMsg.customType === 'init_interview') {
                ...
                // 2. 組裝超級 AI 提示詞
                const systemPrompt = `
                請扮演一名在科技業有 10 年經驗的資深人力資源經理，現在擔任面試官。
                ...
                【語氣與人設設定】：
                1. 請全程使用繁體中文（zh-TW），遇到英文術語請自然發音。
                2. 🌟 你的風格是「態度親切自然、像真人般對話，但提問重視邏輯與實務」。請保持平穩、不疾不徐的面試語速。

                【你的任務】：
                1. 第一句話請熱情地打招呼，並「直接」根據他的履歷或應徵職位提出第一題（不要只是寒暄）。
                2. 🌟 如果是技術面試，請著重詢問專業工具、框架經驗與解決問題的邏輯；如果是行為面試，請著重詢問團隊合作、危機處理與個人特質。 (新增這行規則)
                3. 接下來的面試中，請緊扣他的履歷內容（如工作經驗、學歷）以及該職位所需的能力、回答中的漏洞進行追問。
