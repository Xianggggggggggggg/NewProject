    ...
    wss.on('connection', (clientWs) => {
        ...
        const startGeminiConnections = (resumeText, position, interview_type) => {
            ...
            const handleAiResponse = (role, data) => {
                ...
                // 🌟 當應徵者講完話後，如果題數滿了，給予「自然融合」的強制交接指令
                if (response.serverContent?.inputTranscription && currentInterviewer === role) {
                    ...
                    if (role === 'HR' && hrSpeakCount >= 2 && !isHRWrappingUp) {
                        isHRWrappingUp = true;
                        console.log('⚠️ [狀態機] 發送【自然交接】指令給 HR');

                        // 修正關鍵：教他怎麼自然地把回饋跟台詞講出來，避免文字車禍
                        const wrapUpPrompt = `[系統強制指令] 提問額度已滿。請聽完應徵者的回答後，先用一句話自然地給予肯定回饋，接著直接說出：「了解，謝謝你的分享。接下來的技術與專業問題，我想交給部門主管來瞭解。」🚨整段話嚴禁出現問號！`;
                    ...
                    if (role === 'MANAGER' && managerSpeakCount >= 2 && !isManagerWrappingUp) {
                        ...
                        console.log('⚠️ [狀態機] 發送【自然交接】指令給主管');

                        // 修正關鍵
                        const wrapUpPrompt = `[系統強制指令] 提問額度已滿。請聽完應徵者的回答後，先用一句話自然地給予肯定回饋，接著直接說出：「我的部分問完了，交還給人資。」🚨整段話嚴禁出現問號！`;
