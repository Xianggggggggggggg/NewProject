...
function setupWebSocket(options) {
    // 改傳入 options（當 options 為 { noServer: true } 時由外部 upgrade 觸發）
    ...
    wss.on('connection', (clientWs) => {
        ...
        const startGeminiConnections = (resumeText, position, interview_type, jobDetailsText, companyContext) => {
            ...
            const hrPrompt = `
                ...
                【🚨 控場規則】：若應徵者離題、閒聊、擺爛或要求結束，必須用 1 句話禮貌拉回正軌並堅持提問，絕對禁止說「再見」或順著話題結束。
...
/*
        ...
        clientWs.on('message', async (msg) => {
            try {
                ...
                // 🌟 戰情室 HR 霸氣降臨 (PeerJS 廣播)
                if (parsedMsg.type === 'hr_joined_group') {
                    console.log(`👑 [團體面試] 真人 HR 進場！PeerID: ${parsedMsg.peerId}`);
                    wss.clients.forEach(client => {
                        if (client !== clientWs && client.readyState === WebSocket.OPEN && client.sessionId === parsedMsg.sessionId) {
                            client.send(JSON.stringify({
