...
async function forgotPassword() {
  ...
  if (!email) return; // 如果使用者按取消或沒輸入，就提早結束
  if (!email.includes('@')) return alert("請輸入有效的信箱格式！");

  // 2. 呼叫 Supabase 的密碼重設 API
  const { error } = await supabaseClient.auth.resetPasswordForEmail(email, {
    // 信件中的連結點擊後，要把使用者導向到哪一頁
    // 這裡我們導向到 profile.html，方便他們直接使用裡面的「變更密碼」功能
...
async function previewResume(id) {
  // 1. 先從 resumes 表抓取這份履歷的內容
  const { data: res, error: resError } = await supabaseClient
    .from('resumes')
    .select('*')
    .eq('resume_id', id)
    .single();
    
  if (resError || !res) return alert("讀取履歷失敗");

  // 2. 透過這份履歷的 applicant_id，去 applicants 表抓取真實姓名
  const { data: userData, error: userError } = await supabaseClient
    .from('applicants')
    .select('name')
    .eq('applicant_id', res.applicant_id)
    .single();

  // 如果成功抓到就顯示真實姓名，否則顯示未知
  const realName = (userData && !userError) ? userData.name : '未知姓名';

  // 3. 將自訂名稱與真實姓名一起渲染到畫面上
...
async function renderSetupResumes() {
  ...
  if (!resumes || resumes.length === 0) {
    ...
    resumes.forEach(res => {
      ...
      // 👇 魔法在這裡：加上了 height: auto; 來強制解除 style.css 的拉長限制
      container.innerHTML += `
  ...
  // 👇 新增履歷按鈕：也同步加上 height: auto; 並確保 min-height 是 220px
  container.innerHTML += `
