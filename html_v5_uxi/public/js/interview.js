...
async function startInterviewAI() {
    ...
    try {
        ...
        ws.onopen = () => {
            ...
            transcriptBox.innerHTML += '<div class="ai-msg">系統：連線成功，正在讀取履歷資料與生成考題...</div>';

            // 1. 從網址列抓出所有需要的參數
            const urlParams = new URLSearchParams(window.location.search);
            const sessionId = urlParams.get('session_id'); //[cite: 3]
            const resumeId = urlParams.get('resume_id'); //[cite: 3]
            const position = urlParams.get('position'); //[cite: 3] 取得職位
            
            // 🌟 新增這行：抓取網址列的「面試類型」 (記得前端跳轉網址時要加上 &type=xxx)
            const interviewType = urlParams.get('type') || '行為面試'; 

            if (sessionId && resumeId) {
                // 🌟 2. 傳送「初始化指令」與完整數據給後端
                ws.send(JSON.stringify({
                    customType: 'init_interview', //[cite: 3]
                    sessionId: sessionId, //[cite: 3]
                    resumeId: resumeId, //[cite: 3]
                    position: position || '未指定', //[cite: 3]
                    interview_type: interviewType // 🌟 把面試類型也包進去傳給後端
                }));
                console.log(`🔗 已發送面試初始化資料：職位 [${position}], 類型 [${interviewType}]`);
            } else {
                console.warn("⚠️ 警告：網址列缺乏必要參數！"); //[cite: 3]
            }
        };
