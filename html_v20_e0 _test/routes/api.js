...
function setSessionCookies(res, session) {
    ...
    // 👇 移除 expires 與 maxAge，讓瀏覽器一關閉就自動銷毀 Cookie
...
router.get('/report', async (req, res) => {
    ...
    try {
        ...
        // 🌟 3. 【全新核心】去資料庫撈取「所有應徵者」的報告，計算真實平均與百分位！
        ...
        try {
            ...
            if (allReports && allReports.length > 0) {
                ...
                averages = {
                    tech: calcAvg(scores.tech),
                    comm: calcAvg(scores.comm),
                    logic: calcAvg(scores.logic),
                    learn: calcAvg(scores.learn),
                    stress: calcAvg(scores.stress)
                };

                // 計算真實百分位排名 (Percentile)
                const calcPct = (arr, myScore) => {
                    if (arr.length === 0) return 50;
                    const below = arr.filter(s => s < myScore).length; // 贏過多少人
                    const equal = arr.filter(s => s === myScore).length; // 和多少人平手
                    const pct = ((below + (0.5 * equal)) / arr.length) * 100; // 常態百分位公式
                    return Math.min(Math.max(Math.round(pct), 1), 99); // 限制在 1~99 之間
                };
            ...
            console.error("⚠️ 計算平均值與百分位失敗，將使用預設值:", statsErr.message);
        }

        // 🎁 4. 全部打包成一個大物件送給前端 (加上了 stats)
        res.json({
            report: reportData, 
            emotionData: {
            ...
            stats: { averages, percentiles } // 🚀 傳送算好的真實統計數據！
        });
    } catch (err) {
